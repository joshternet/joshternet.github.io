(() => {
    ...
    const filters = Array.from(document.querySelectorAll("[data-network-filter]"));
    ...
    const requestedSite = normalizeRequestedSite(
        params.get("site"),
    );
    ...
    const targetedCard = requestedSite
        ? cards.find((card) => {
            return (
                (card.dataset.domain || "").toLowerCase() ===
                requestedSite
            );
        })
        : null;
    ...
    function apply() {
        ...
        if (targetedCard && !targetRevealed) {
            ...
            window.requestAnimationFrame(() => {
                ...
                const link = targetedCard.querySelector(
                    ".network-card__link",
                );
