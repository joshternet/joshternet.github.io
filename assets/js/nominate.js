...
if (form) {
  ...
  const submitButton = form.querySelector("[data-seed-nomination-submit]");
  const statusElement = form.querySelector("[data-seed-nomination-status]");
  ...
  function initializeTurnstile() {
    if (!apiUrl || !sitekey) {
      showStatus("Seed nominations are not available yet.", "error");
      return;
    }
    ...
    widgetId = window.turnstile.render(turnstileContainer, {
      ...
      "expired-callback"() {
        setVerificationToken("");
        showStatus("Verification expired. Please complete it again.", "error");
      },
  ...
  form.addEventListener("submit", async (event) => {
    ...
    try {
      ...
      } else if (response.ok && result?.status === "already_nominated") {
        urlField.value = "";
        ownerField.checked = false;
