  ...
  if (source.startsWith("https://") || source.startsWith("http://")) {
    const response = await fetch(source, {
      headers: {
        ...
        "User-Agent": "Joshternet-Network-Preflight",
      },
      redirect: "error",
...
async function screenshotExists(entry) {
  ...
  try {
    const stat = await fs.stat(path.resolve(relative));
...
let needsSync = registryNeedsSync(participants, existing);
...
if (needsSync) {
  ...
  process.stderr.write("Network registry and captures are current.\n");
