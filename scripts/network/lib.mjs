...
export function canonicalOrigin(value) {
    ...
    if (
        ...
        throw new Error("registry origin must not contain path, query, or fragment");
    }
...
export function framePolicy({
    ...
    if (xFrameOptions) {
        const directives = xFrameOptions
            .split(",")
            .map((value) => value.trim());

        if (
            directives.includes("deny") ||
            directives.includes("sameorigin")
        ) {
            return {
                embeddable: false,
    ...
    const csp =
        normalizedHeaders.get("content-security-policy") || "";
...
export async function assertPublicURL(
    ...
    for (const result of addresses) {
        if (!result || isForbiddenAddress(result.address)) {
            throw new Error(
                `hostname resolves to prohibited address: ${hostname}`,
            );
        }
    }
